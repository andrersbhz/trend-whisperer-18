...
import { CalendarClock, CheckCircle2, ExternalLink, Facebook, FileText, Instagram, Link2, Loader2, Megaphone, MessageSquareText, RefreshCw, Rocket, Send, Wordpress } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between border-b border-border/60 pb-5">
        <div>
          <p className="text-[11px] uppercase tracking-[0.2em] font-semibold text-muted-foreground">AutoPostWP 1.5</p>
          <h1 className="text-2xl font-semibold tracking-tight mt-1">Central de Distribuição</h1>
          <p className="text-sm text-muted-foreground mt-2 max-w-3xl">Crie uma vez e distribua direto para as redes. WordPress é opcional; quando usado, a URL canônica vira o destino das campanhas sociais com UTM individual por rede.</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
        <Card className="border-border/60">
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><Rocket className="h-4 w-4"/>Nova distribuição</CardTitle></CardHeader>
          <CardContent className="space-y-5">
            <Tabs value={sourceMode} onValueChange={(v)=>setSourceMode(v as any)}>
              <TabsList><TabsTrigger value="article"><FileText className="h-4 w-4 mr-1.5"/>Artigo do sistema</TabsTrigger><TabsTrigger value="custom"><Megaphone className="h-4 w-4 mr-1.5"/>Post direto</TabsTrigger></TabsList>
              <TabsContent value="article" className="space-y-4 mt-4">
                <div className="space-y-2"><Label>Artigo</Label><select className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm" value={selectedArticle} onChange={(e)=>setSelectedArticle(e.target.value)}><option value="">Selecione...</option>{articles.map((a)=><option key={a.id} value={a.id}>{a.title}</option>)}</select></div>
                <button type="button" onClick={()=>setPublishWordPress(!publishWordPress)} className={`w-full rounded-lg border p-3 text-left transition-colors ${publishWordPress ? 'border-primary bg-primary/5' : 'border-border/60'}`}>
                  <div className="flex items-center gap-3"><Wordpress className="h-5 w-5"/><div className="flex-1"><p className="text-sm font-semibold">Publicar também no WordPress</p><p className="text-xs text-muted-foreground">Publica/obtém a URL canônica antes de montar os links sociais.</p></div><span className={`text-xs font-bold ${publishWordPress ? 'text-emerald-500' : 'text-muted-foreground'}`}>{publishWordPress ? 'ATIVO' : 'OPCIONAL'}</span></div>
                </button>
              </TabsContent>
              <TabsContent value="custom" className="space-y-4 mt-4">
                <div className="rounded-lg border border-dashed p-3 text-xs text-muted-foreground">Modo independente do WordPress. Informe uma URL do blog, landing page ou conteúdo externo e publique direto nas redes.</div>
              </TabsContent>
            </Tabs>
